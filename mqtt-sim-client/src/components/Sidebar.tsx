...
export default function Sidebar({ activePanel, setActivePanel }: SidebarProps){
  return (
    <nav className="d-flex flex-column align-items-center bg-body border-end border-2 pt-2" 
      ...
      <button
        className={`w-100 d-flex align-items-center justify-content-center btn p-0 border-0 ${activePanel === 'broker' ? 'bg-primary bg-opacity-10 border-start border-4 border-primary' : ''}`}
